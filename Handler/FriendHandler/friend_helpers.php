<?php
/**
 * File này chứa các hàm helper toàn cục để render modal (popup)
 * được sử dụng trên nhiều trang.
 */

/**
 * ------------------------------------------------------------------
 * HÀM MỚI 1: CHỈ RENDER CSS
 * ------------------------------------------------------------------
 * In ra CSS cho các modal toàn cục.
 * Nên được gọi bên trong thẻ <head> của trang.
 */
function global_render_css() {
    // 1. CSS CHO OVERLAY (Sử dụng tiền tố "g-" (global) để tránh xung đột)
    echo '
<style>
/* ... (CSS cho g-profile-overlay giữ nguyên) ... */
#g-profile-overlay {
...
/* CSS CHO POPUP XÁC NHẬN (CONFIRM) */
#g-confirm-overlay {
    position: fixed; inset: 0;
...
/* === CSS MỚI CHO POPUP THÔNG BÁO (ALERT) === */
#g-alert-overlay {
    position: fixed; inset: 0;
    ...
    justify-content: center;
    align-items: center;
    z-index: 1002; /* Nằm trên cả popup confirm */
    font-family: Arial, sans-serif;
}
.g-alert-box {
    background: #fff;
...
.g-alert-actions {
    display: flex;
    justify-content: center; /* Chỉ có 1 nút */
}
.g-alert-actions button {
    padding: 10px 20px;
    font-size: 14px;
    border: none;
    border-radius: 8px;
...
/**
 * ------------------------------------------------------------------
 * HÀM MỚI 2: RENDER HTML MODALS VÀ JAVASCRIPT
 * ------------------------------------------------------------------
 * In ra HTML và JavaScript cho các modal toàn cục.
 * Nên được gọi ở cuối thẻ <body>.
 *
 * @param string $api_handler_path Đường dẫn đến file friend-handler.php.
 * @param string $default_avatar_path Đường dẫn đến ảnh avatar mặc định.
 * @param string $base_path Đường dẫn gốc của web (ví dụ: /ChatApp).
 */
function global_render_modals_and_js(
...
<div id="g-confirm-overlay" onclick="hideGlobalConfirm()">
    <div class="g-confirm-box" onclick="event.stopPropagation()">
        ...
        <div class="g-confirm-actions">
            <button type="button" id="g-confirm-btn-cancel" class="g-reject">Hủy</button>
            <button type="button" id="g-confirm-btn-ok" class="g-normal">Xác nhận</button>
        </div>
    </div>
</div>

<div id="g-alert-overlay" onclick="hideGlobalAlert()">
    <div class="g-alert-box" onclick="event.stopPropagation()">
        <p id="g-alert-message">Đây là thông báo.</p>
        <div class="g-alert-actions">
            <button type="button" id="g-alert-btn-ok" class="g-normal">OK</button>
        </div>
    </div>
</div>
';
    ...
    echo '
<script>
    // === PHẦN 1: ĐỊNH NGHĨA BIẾN VÀ HÀM TOÀN CỤC ===
    // (Đưa ra ngoài khối "if" để các file khác có thể thấy)

    // Biến toàn cục trỏ đến các thành phần
    var G_API_HANDLER = "' . $safe_api_path . '";
    var G_DEFAULT_AVATAR = "' . $safe_avatar_path . '";
    var G_BASE_PATH = "' . $safe_base_path . '";

    // Biến DOM (sẽ được gán giá trị ở PHẦN 2)
    var G_PROFILE_OVERLAY, G_CONFIRM_OVERLAY, G_CONFIRM_MESSAGE, G_CONFIRM_BTN_OK, G_CONFIRM_BTN_CANCEL;
    var G_ALERT_OVERLAY, G_ALERT_MESSAGE, G_ALERT_BTN_OK;
    ...
    // === CÁC HÀM POPUP XÁC NHẬN (CONFIRM) ===
    ...
    // === CÁC HÀM POPUP THÔNG BÁO (ALERT) MỚI ===
    function showGlobalAlert(message) {
        if (!G_ALERT_OVERLAY) {
            alert(message); // Fallback
            return;
        }
        G_ALERT_MESSAGE.textContent = message;
        G_ALERT_OVERLAY.style.display = "flex";
    }
    
    function hideGlobalAlert() {
        if (!G_ALERT_OVERLAY) return;
        G_ALERT_OVERLAY.style.display = "none";
    }
    ...
    async function toggleGlobalProfile(userId, show = true) {
        ...
        if (res.status === "success" && res.data) {
            ...
            document.getElementById("g-infoEmail").textContent = g_formatInfo(data.Email);
            document.getElementById("g-infoPhone").textContent = g_formatInfo(data.PhoneNumber);
            document.getElementById("g-infoAddress").textContent = g_formatInfo(data.Address);
            ...
            showGlobalAlert("Lỗi: " + (res.message || "Không thể tải thông tin."));
    ...
    async function g_sendFriend(id, btn) {
        ...
        if (res.status === "sent") {
            ...
            showGlobalAlert(res.message || "Đã có lỗi xảy ra.");
    ...
    async function g_cancelRequest(id, btn) {
        ...
        if (res.status === "success") {
            ...
            showGlobalAlert("Lỗi: Không thể hủy lời mời.");
            g_updateAllUserButtons(id, "sent_by_me");
        }
    }
    ...
    async function g_unfriend(id) {
        ...
        showGlobalConfirm(message, async () => {
            ...
            if (res.status === "success") {
                ...
                showGlobalAlert("Lỗi: Không thể hủy kết bạn.");
                g_updateAllUserButtons(id, "already_friends"); 
            }
        });
    }

    // === PHẦN 2: KHỞI CHẠY (CHẠY MỘT LẦN KHI DOM SẴN SÀNG) ===
    // Khối này gán các biến DOM và sự kiện .onclick
    if (typeof g_global_helpers_loaded === "undefined") {
        const g_global_helpers_loaded = true;

        // Gán các biến DOM
        G_PROFILE_OVERLAY = document.getElementById("g-profile-overlay");
        
        G_CONFIRM_OVERLAY = document.getElementById("g-confirm-overlay");
        G_CONFIRM_MESSAGE = document.getElementById("g-confirm-message");
        G_CONFIRM_BTN_OK = document.getElementById("g-confirm-btn-ok");
        G_CONFIRM_BTN_CANCEL = document.getElementById("g-confirm-btn-cancel");
        
        G_ALERT_OVERLAY = document.getElementById("g-alert-overlay");
        G_ALERT_MESSAGE = document.getElementById("g-alert-message");
        G_ALERT_BTN_OK = document.getElementById("g-alert-btn-ok");

        // Gán sự kiện cho các nút của popup xác nhận
        ...
        // Gán sự kiện cho nút OK của alert
        if (G_ALERT_BTN_OK) {
            G_ALERT_BTN_OK.onclick = hideGlobalAlert;
        }
    }
</script>';
}
 ...
 * ------------------------------------------------------------------
 * Hàm này giờ sẽ gọi cả hai hàm mới, đảm bảo code cũ của bạn
 * (như trong chat.php) vẫn chạy mà không cần sửa.
 */
function render_global_profile_modal(
    ...
    global_render_css();
    global_render_modals_and_js($api_handler_path, $default_avatar_path, $base_path);
}
?>